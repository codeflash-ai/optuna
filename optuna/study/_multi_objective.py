...
def _is_pareto_front_nd(unique_lexsorted_loss_values: np.ndarray) -> np.ndarray:
    ...
    while remaining_indices.size > 0:
        ...
        on_front[idx] = True
        lv_idx = loss_values[idx]
        # Vectorized comparison to avoid unnecessary copying and iteration
        # Fast boolean mask: a trial is not dominated if for some objective it is strictly less.
        # All trials in remaining_indices except the current one
        mask = np.any(loss_values[remaining_indices] < lv_idx, axis=1)
        # Exclude the currently selected (first) index to ensure progression.
...
def _is_pareto_front_for_unique_sorted(unique_lexsorted_loss_values: np.ndarray) -> np.ndarray:
    ...
    if n_objectives == 1:
        # No need to call len(), n_trials is already available
        on_front = np.zeros(n_trials, dtype=bool)
