...
def _get_constraint_vals_and_feasibility(
    ...
    get_system_attrs = study._storage.get_trial_system_attrs
    _constraint_vals = [
        get_system_attrs(trial._trial_id).get(_CONSTRAINTS_KEY, ())
        for trial in trials
    ]
    # Optimize length check to single pass across all constraints
    first_len = len(_constraint_vals[0]) if _constraint_vals else 0
    if len({len(c) for c in _constraint_vals}) > 1:
        raise ValueError("The number of constraints must be the same for all trials.")

    constraint_vals = np.array(_constraint_vals, dtype=float)
