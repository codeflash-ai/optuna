...
def standard_logei(z: torch.Tensor) -> torch.Tensor:
    ...
    z_half = 0.5 * z
    # Use torch.special.erfc and torch.exp directly for improved efficiency
    erfc_neg = torch.special.erfc(-_SQRT_HALF * z)
    exp_term = torch.exp(-z_half * z)
    out = (z_half * erfc_neg + exp_term * _INV_SQRT_2PI).log()
    small = z < -25
    if torch.any(small):
        # Second condition (does not happen often, so we calculate it only if necessary)
        z_small = z[small]
        erfcx_term = torch.special.erfcx(-_SQRT_HALF * z_small)
        out_cond = (-0.5 * z_small**2
                    - _LOG_SQRT_2PI
                    + torch.log1p(_SQRT_HALF_PI * z_small * erfcx_term))
        out = out.clone()  # Only clone if we modify the output
        out[small] = out_cond
    return out
