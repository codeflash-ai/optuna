...
def standard_logei(z: torch.Tensor) -> torch.Tensor:
    ...
    """
    # Avoid unnecessary repeated computation and intermediate allocations via in-place ops and fused calls
    z_half = 0.5 * z
    # Cache -_SQRT_HALF * z
    neg_sqrt_half_z = -_SQRT_HALF * z
    # Fused computation for main branch using torch API
    # Only for z elements not very small (< -25); we overwrite small elements below
    out = (z_half * torch.special.erfc(neg_sqrt_half_z) + (-z_half * z).exp() * _INV_SQRT_2PI).log()
    # Mask for small z values
    small = z < -25
    if torch.any(small):
        z_small = z[small]
        neg_sqrt_half_z_small = -_SQRT_HALF * z_small
        # More numerically stable computation for very small z
        out[small] = (
            -0.5 * z_small ** 2
            - _LOG_SQRT_2PI
            + (1 + _SQRT_HALF_PI * z_small * torch.special.erfcx(neg_sqrt_half_z_small)).log()
        )
    return out

def logei(mean: torch.Tensor, var: torch.Tensor, f0: float) -> torch.Tensor:
    # Avoid intermediate temporaries and duplicate computation by storing sigma
    sigma = var.sqrt_()
    z = (mean - f0) / sigma
    return standard_logei(z) + sigma.log()
