...
class TrialSystemAttributeModel(BaseModel):
    ...
    def where_trial_id(cls, trial_id: int, session: orm.Session) -> list['TrialSystemAttributeModel']:
        # Use bindparam to minimize SQL string interpolation overhead and leverage compiled caching.
        # Utilize the optimized select() + execute() + scalars().all() pattern for better performance
        from sqlalchemy import bindparam, select

        stmt = select(cls).where(cls.trial_id == bindparam("trial_id"))
        return list(session.scalars(stmt, {"trial_id": trial_id}))
