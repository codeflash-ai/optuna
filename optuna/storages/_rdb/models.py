...
from sqlalchemy import (CheckConstraint, DateTime, Enum, Float, ForeignKey,
                        Integer, String, Text, UniqueConstraint, asc, case,
                        desc, func, orm)
...
class StudyModel(BaseModel):
    ...
    study_name = _Column(String(MAX_INDEXED_STRING_LENGTH), index=True, unique=True, nullable=False)
...
class StudyDirectionModel(BaseModel):
    ...
    study = orm.relationship(StudyModel, backref=orm.backref('directions', cascade='all, delete-orphan'))
...
class StudyUserAttributeModel(BaseModel):
    ...
    @codeflash_capture(function_name='StudyUserAttributeModel.__init__', tmp_dir_path='/tmp/codeflash_b3ter4au/test_return_values', tests_root='/home/ubuntu/work/repo/tests', is_fto=True)
    ...
    study = orm.relationship(StudyModel, backref=orm.backref('user_attributes', cascade='all, delete-orphan'))

    @classmethod
    def find_by_study_and_key(cls, study: StudyModel, key: str, session: orm.Session) -> 'StudyUserAttributeModel' | None:
        attribute = session.query(cls).filter(cls.study_id == study.study_id).filter(cls.key == key).one_or_none()
        return attribute

    @classmethod
    def where_study_id(cls, study_id: int, session: orm.Session) -> list['StudyUserAttributeModel']:
        return session.query(cls).filter_by(study_id=study_id).all()
...
class StudySystemAttributeModel(BaseModel):
    ...
    key = _Column(String(MAX_INDEXED_STRING_LENGTH))
    value_json = _Column(Text())
    study = orm.relationship(StudyModel, backref=orm.backref('system_attributes', cascade='all, delete-orphan'))
...
class TrialModel(BaseModel):
    ...
    study = orm.relationship(StudyModel, backref=orm.backref('trials', cascade='all, delete-orphan'))

    @classmethod
    def find_max_value_trial_id(cls, study_id: int, objective: int, session: orm.Session) -> int:
        trial = session.query(cls).with_entities(cls.trial_id).filter(cls.study_id == study_id).filter(cls.state == TrialState.COMPLETE).join(TrialValueModel).filter(TrialValueModel.objective == objective).order_by(desc(case((TrialValueModel.value_type == TrialValueModel.TrialValueType.INF_NEG, -1), (TrialValueModel.value_type == TrialValueModel.TrialValueType.FINITE, 0), (TrialValueModel.value_type == TrialValueModel.TrialValueType.INF_POS, 1))), desc(TrialValueModel.value)).limit(1).one_or_none()
        if trial is None:
    ...
    def find_min_value_trial_id(cls, study_id: int, objective: int, session: orm.Session) -> int:
        trial = session.query(cls).with_entities(cls.trial_id).filter(cls.study_id == study_id).filter(cls.state == TrialState.COMPLETE).join(TrialValueModel).filter(TrialValueModel.objective == objective).order_by(asc(case((TrialValueModel.value_type == TrialValueModel.TrialValueType.INF_NEG, -1), (TrialValueModel.value_type == TrialValueModel.TrialValueType.FINITE, 0), (TrialValueModel.value_type == TrialValueModel.TrialValueType.INF_POS, 1))), asc(TrialValueModel.value)).limit(1).one_or_none()
        if trial is None:
            raise ValueError(NOT_FOUND_MSG)
        return trial[0]
    ...
    def count_past_trials(self, session: orm.Session) -> int:
        trial_count = session.query(func.count(TrialModel.trial_id)).filter(TrialModel.study_id == self.study_id, TrialModel.trial_id < self.trial_id)
        return trial_count.scalar()
...
class TrialUserAttributeModel(BaseModel):
    ...
    key = _Column(String(MAX_INDEXED_STRING_LENGTH))
    value_json = _Column(Text())
    trial = orm.relationship(TrialModel, backref=orm.backref('user_attributes', cascade='all, delete-orphan'))

    @classmethod
    def find_by_trial_and_key(cls, trial: TrialModel, key: str, session: orm.Session) -> 'TrialUserAttributeModel' | None:
        attribute = session.query(cls).filter(cls.trial_id == trial.trial_id).filter(cls.key == key).one_or_none()
        return attribute
...
class TrialSystemAttributeModel(BaseModel):
    ...
    key = _Column(String(MAX_INDEXED_STRING_LENGTH))
    value_json = _Column(Text())
    trial = orm.relationship(TrialModel, backref=orm.backref('system_attributes', cascade='all, delete-orphan'))
...
class TrialParamModel(BaseModel):
    ...
    trial = orm.relationship(TrialModel, backref=orm.backref('params', cascade='all, delete-orphan'))
    ...
    def _check_compatibility_with_previous_trial_param_distributions(self, session: orm.Session, study_id: int) -> None:
        previous_record = session.query(TrialParamModel).join(TrialModel).filter(TrialModel.study_id == study_id).filter(TrialParamModel.param_name == self.param_name).first()
        if previous_record is not None:
            distributions.check_distribution_compatibility(distributions.json_to_distribution(previous_record.distribution_json), distributions.json_to_distribution(self.distribution_json))

    @classmethod
    def find_by_trial_and_param_name(cls, trial: TrialModel, param_name: str, session: orm.Session) -> 'TrialParamModel' | None:
        param_distribution = session.query(cls).filter(cls.trial_id == trial.trial_id).filter(cls.param_name == param_name).one_or_none()
...
class TrialValueModel(BaseModel):
    ...
    trial = orm.relationship(TrialModel, backref=orm.backref('values', cascade='all, delete-orphan'))
    ...
    def stored_repr_to_value(cls, value: float | None, float_type: TrialValueType) -> float:
        # Localize to avoid repeated attribute lookups
        inf_pos = cls.TrialValueType.INF_POS
        inf_neg = cls.TrialValueType.INF_NEG
        finite = cls.TrialValueType.FINITE

        if float_type is inf_pos:
            assert value is None
            return float('inf')
        elif float_type is inf_neg:
            assert value is None
            return float('-inf')
        else:
            assert float_type is finite
            assert value is not None
            return value

    @classmethod
    def find_by_trial_and_objective(cls, trial: TrialModel, objective: int, session: orm.Session) -> 'TrialValueModel' | None:
        trial_value = session.query(cls).filter(cls.trial_id == trial.trial_id).filter(cls.objective == objective).one_or_none()
        return trial_value

    @classmethod
    def where_trial_id(cls, trial_id: int, session: orm.Session) -> list['TrialValueModel']:
        trial_values = session.query(cls).filter(cls.trial_id == trial_id).order_by(asc(cls.objective)).all()
        return trial_values
...
class TrialIntermediateValueModel(BaseModel):
    ...
    trial = orm.relationship(TrialModel, backref=orm.backref('intermediate_values', cascade='all, delete-orphan'))

    @classmethod
    def intermediate_value_to_stored_repr(cls, value: float) -> tuple[float | None, TrialIntermediateValueType]:
        # Cache class attributes once per class (not per call)
        # This has a measurable performance gain for very hot functions.

        # Static/class attribute dict
        _cache = getattr(cls, '_intermediate_value_type_cache', None)
        if _cache is None:
            TrialIntermediateValueType = cls.TrialIntermediateValueType
            _cache = (
                TrialIntermediateValueType.FINITE,
                TrialIntermediateValueType.NAN,
                TrialIntermediateValueType.INF_POS,
                TrialIntermediateValueType.INF_NEG,
            )
            setattr(cls, '_intermediate_value_type_cache', _cache)
        finite, nan, inf_pos, inf_neg = _cache
    ...
    @classmethod
    def stored_repr_to_intermediate_value(cls, value: float | None, float_type: TrialIntermediateValueType) -> float:
        t = cls.TrialIntermediateValueType
        # Fast-path dispatch table, avoid repeated attribute lookups and use local vars
        # This also shortens the chain of conditionals to constant time lookup
        if float_type is t.FINITE:
            assert value is not None
            return value
        assert value is None
        if float_type is t.NAN:
            return float('nan')
        elif float_type is t.INF_POS:
            return float('inf')
        else:
            assert float_type is t.INF_NEG
            return float('-inf')

    @classmethod
    def find_by_trial_and_step(cls, trial: TrialModel, step: int, session: orm.Session) -> 'TrialIntermediateValueModel' | None:
        trial_intermediate_value = session.query(cls).filter(cls.trial_id == trial.trial_id).filter(cls.step == step).one_or_none()
...
class TrialHeartbeatModel(BaseModel):
    ...
    trial = orm.relationship(TrialModel, backref=orm.backref('heartbeats', cascade='all, delete-orphan'))

    @classmethod
    def where_trial_id(cls, trial_id: int, session: orm.Session, for_update: bool=False) -> 'TrialHeartbeatModel' | None:
        query = session.query(cls).filter(cls.trial_id == trial_id)
        if for_update:
            query = query.with_for_update()
